...
const GroupManagement = () => {
  ...
  const getRealizedEncontros = () => {
    return (group?.encontros || []).filter((e) => e.status === 'REALIZADO').length
  }

  const getTotalEncontrosNotCanceled = () => {
    return (group?.encontros || []).filter((e) => e.status !== 'CANCELADO').length
  }
                      ...
                      <TableRow key={encontro.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <StatusBadge status={encontro.status || 'PENDENTE'} />
                            ...
                            <DropdownMenuContent>
                              <DropdownMenuGroup>
                                <Button
                                  ...
                                  variant="ghost"
                                  disabled={
                                    encontro.status != 'PENDENTE' || session?.user?.role?.level < 2
                                  }
